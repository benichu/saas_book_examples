    ...
    context "with plans" do
      ...
      let(:query_string) do
        Rack::Utils.build_query(
          ...
          :hash => "adb75b85a9e019407fc7c534257648eeaf3109e8"
        )
      end
      ...
      scenario "updating an account's plan" do
        mock_transparent_redirect_response = stub(:success? => true)
        mock_transparent_redirect_response.stub_chain(:customer, :credit_cards).
          and_return([stub(:token => "abcdef")])
        ...
        subscription_result = stub(:success? => true,
                                   :subscription => stub(:id => "abc123"))
      ...
      scenario "can't change account's plan with invalid credit card number" do
        ...
        result = stub(:success? => false, :message => message)
